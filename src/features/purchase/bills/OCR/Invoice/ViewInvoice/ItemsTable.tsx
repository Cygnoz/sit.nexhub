...
function ItemsTable({ items }: Props) {
  ...
  // const dropdownRefs = useRef<(HTMLDivElement | null)[]>([]); // Ref for each dropdown
  const tableRef = useRef<HTMLTableElement | null>(null); // Ref for the table to detect clicks outside
  ...
  return (
    <div className="overflow-x-auto hide-scrollbar h-[300px]">
      <table ref={tableRef} className="min-w-full border-collapse text-xs text-textColor h-[100px]">
        <thead>
          <tr>
                ...
                <div
                  className={`border px-4 py-3 whitespace-nowrap cursor-pointer relative ${
                    openDropdownIndex === index
                      ? "border-x-darkRed border-y-darkRed bg-lightPink -m-0.5 mb-0.5"
                      : "border-[#F4F4F4] -m-0.5"
                  }`}
                >
                  {item.product}
                  {openDropdownIndex === index && (
                    <div
                      className="absolute z-10 w-[200%] -ms-5 shadow-2xl bg-white rounded-md mt-3.5 p-2 space-y-1 max-h-72 overflow-y-auto hide-scrollbar"
                    >
