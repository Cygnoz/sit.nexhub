...
function NewRecieptTable({ customerReciept = [], recieptState, setRecieptState }: Props) {
    ...
    const handleRowChange = (
        ...
        const newData = [...data];
        const invoiceAmount = newData[index].totalAmount;
...
<div>
                  ...
                  <tr key={index} className="relative">
                    ...
                    <td className="py-2.5 px-4 border-y border-tableBorder">
                      <input
                        ...
                        placeholder="Bill ID"
                        className="w-full focus:outline-none text-center"
                        value={row.invoiceId ? row.invoiceId : "-"}
                        onChange={(e) =>
                          handleRowChange(index, "invoiceId", e.target.value)
                        }
                      />
                    </td>
                    <td className="py-2.5 px-4 border-y border-tableBorder">
      ...
      <p className="text-right text-textColor text-sm mt-4">
        Total <span className="ms-20 font-semibold">{recieptState.total}</span>
      </p>
    </div>
    )
}
