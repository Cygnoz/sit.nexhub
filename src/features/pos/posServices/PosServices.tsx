...
function PosServices({items}: Props) {
  const services: Service[] = [
    { name: "All", image: serviceImage },
    { name: "Product Repair Services", image: serviceImage },
    { name: "Installation Services", image: serviceImage },
    { name: "Technical Support", image: serviceImage },
    { name: "Warranty Services", image: serviceImage },
    { name: "Product Customization", image: serviceImage },
  ];
  ...
  const filteredServices = items.filter((service: any) =>
    service.itemName?.toLowerCase().includes(searchValue.toLowerCase())
  );
  ...
  return (
    <>
      ...
      {/* <div className="mt-5">
        <p className="text-textColor font-bold text-base">Frequently Moving Service</p>
      </div> */}
    </>
  );
}
