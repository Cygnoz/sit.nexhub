...
const transformRetentionData = (retentionData: { date: string; retentionRate: number }[]) => {
  ...
  retentionData.forEach(({ date, retentionRate }) => {
    const day = parseInt(date.split("-")[2]); // Extract day from "YYYY-MM-DD"
    if (day >= 1 && day <= 5) groupedData["0-5"].push(retentionRate);
    else if (day >= 6 && day <= 10) groupedData["5-10"].push(retentionRate);
    else if (day >= 11 && day <= 15) groupedData["10-15"].push(retentionRate);
    else if (day >= 16 && day <= 20) groupedData["15-20"].push(retentionRate);
    else if (day >= 21 && day <= 25) groupedData["20-25"].push(retentionRate);
    else groupedData["25-31"].push(retentionRate);
  });
...
const CustomersRetentionRate: React.FC<CustomersRetentionRateProps> = ({ date }) => {
  const { request: getRentation } = useApi("get", 5002);
  const [cusRentation, setCusRentation] = useState<any>([]);

  const getCusRentation = async () => {
    try {
      const { response, error } = await getRentation(
        `${endponits.CUST_DASH_RETENTION_RATE}?date=${date}`
      );
      if (response && !error) {
        console.log("API Response:", response?.data?.dailyRetention);
        
        // Transform API response
        const formattedData = transformRetentionData(response?.data?.dailyRetention);
