...
const TopCategories: React.FC<TopCustomersProps> = ({ date }) => {
  const { request: getTopCategories } = useApi("get", 5008);
  const [topCategories, setTopCategories] = useState<any>([]);

  const getTopCategory = async () => {
    try {
      const { response, error } = await getTopCategories(
        `${endponits.EXPENSE_DASH_TOP_EXPENSE_BY_CATEGORY}?date=${date}`
      );
      if (response && !error) {
        console.log("top", response.data);
        
        // Transform API response into required format
        const formattedData = response?.data?.topExpenseByCategory?.map((exp: any) => ({
          name: exp.category,
          value: exp.totalExpense,
          // img:BackgroundImage
        }));
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={topCategories}>
          ...
          <Bar shape={<CustomBar />} barSize={40} dataKey="value" fill="#8884d8">
            ...
            {topCategories.map((_:any, index:any) => (
